...
- If the black background doesn't contain any graphical controls, then use the background color also as the tab color
- `5px` separate colored sections (including from the border)
...
- The device height is `170px` (e.g., often the correct height to use is `169px` in order to leave a `1px` empty border at the bottom which matches the design of most built-in devices)
